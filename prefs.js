...
export default class ThanatophobiaPreferences extends ExtensionPreferences {
    getPreferencesWidget() {
        let settings = this.getSettings(GS_SCHEMA);
        ...
        let expectancyLabel = new Gtk.Label({
            label: getExpectancyString(settings.get_double(GS_KEY_LIFE_EXPECTANCY), settings.get_int(GS_KEY_LIFE_EXPECTANCY_YEAR)),
        ...
        let birthdateEntry = new Gtk.Calendar({
                year: settings.get_int(GS_KEY_YEAR),
                month: settings.get_int(GS_KEY_MONTH) - 1,
                day: settings.get_int(GS_KEY_DAY),
                halign: Gtk.Align.START,
                visible: true
            });
        // Birthdate hours
        ...
        recalculateButton.connect("clicked", () => {
            ...
            const getData = yearOffset => {
                ...
                if (yearOffset > 20) {
                    expectancyLabel.set_text("Could not fetch data, using previous: " + settings.get_double(GS_KEY_LIFE_EXPECTANCY) + "\nDid you input a correct ISO alpha-3 country code?")
                    return;
                }
                ...
                GET(`https://apps.who.int/gho/athena/api/GHO/WHOSIS_000001.json?profile=simple&filter=COUNTRY:${country};YEAR:${year};SEX:${sex}`, (body) => {
                    if (body) {
                        ...
                        try {
                            ...
                            // Update label
                            expectancyLabel.set_text(getExpectancyString(settings.get_double(GS_KEY_LIFE_EXPECTANCY), settings.get_int(GS_KEY_LIFE_EXPECTANCY_YEAR)))
                        } catch (e) {
